        ...
        <div className="md:w-1/2 flex flex-col gap-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded font-semibold transition mt-2 ${
              inWishlist ? "bg-red-500 text-white" : "bg-gray-200 text-gray-800 hover:bg-gray-300"
            }`}
          >
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 px-4 py-2 rounded font-semibold transition ${
                added ? "bg-gray-400 text-black hover:bg-gray-500" : "bg-blue-600 text-white hover:bg-blue-700"
              }`}
            >
          ...
          <div className="mt-6">
            ...
            {reviews.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {reviews.map((r, idx) => (
                  <div key={idx} className="border rounded-lg p-4 shadow hover:shadow-md transition flex flex-col gap-2 bg-white">
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="font-semibold">{r.reviewerName || r.user}</span>
                        <span className="text-gray-500 text-sm">{r.reviewerEmail || "N/A"}</span>
                      </div>
                      <div className="flex">
                        {Array.from({ length: 5 }, (_, i) => (
                    ...
                    <p className="text-gray-400 text-sm">{r.date ? new Date(r.date).toLocaleDateString() : ""}</p>
                  </div>
                ))}
              </div>
            )}
